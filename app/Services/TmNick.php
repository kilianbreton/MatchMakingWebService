...
/**
* Cette classe gÃ¨re les chaines TMF, c'est Ã  dire que cette classe permet de convertir une chaine TM en:
*   - html (ombre et lien sont gÃ©rÃ©)
*   - texte brut -> sans la mise en forme
*   - image (tout format supportÃ©, image d'arriÃ¨re plan ou pas)
*
...
class TmNick {
	...
	* $g Couleur par dÃ©faut
	* $z Tout rÃ©initialiser
	* $$ Ecrire un "$"
	...
	*
	* @param string $nick                   -> La chaine TM pas de limite de longueur, et peut Ãªtre multiligne
	* @param int    $fontSize               -> Taille de la police
	* @param bool   $forcedDefault          -> Force la couleur par dÃ©faut Ã  Ãªtre utilise Ã  la place de la mise en forme
	* @param string $defaultColor           -> La couleur par dÃ©faut (la couleur peut Ãªtre au format HTML, nommÃ©, array RGB, ou TM)
	* @param string $defaultBackgroundColor -> La couleur de fond (transparent est possible) (la couleur peut Ãªtre au format HTML, nommÃ©, array RGB, ou TM)
	* @return string chaine HTML sans le CSS
	*/
	public static function toHtml($nick, $fontSize = 11, $forcedShadow = false, $forcedDefault = false, $defaultColor = 'white', $defaultBackgroundColor = 'transparent', $defaultClass='nav-link'){
		...
		$strHtml = '<span class="tmnick_global '.$defaultClass .'" style="background-color:'.$defaultHTMLBackgroundColor.'">';
		$lastLink = false;
		$lastLinkTag = false;
	...
	* @param int    $fontSize             -> Taille de la police
	* @param bool   $forceDefault         -> Force la couleur par dÃ©faut Ã  Ãªtre utilise Ã  la place de la mise en forme
	* @param string $defaultColor         -> La couleur par dÃ©faut (la couleur peut Ãªtre au format HTML, nommÃ©, array RGB, ou TM)
	* @param string $defaultImgBackground -> Le chemin vers l'image de fond si vous en voulez une
	* @param array  $arrRelativePosition  -> Tableau assoc avec les index 'x' et 'y', donnant la position du texte par rapport au bord gauche et/ou haut de l'image de fond, ces valeur peuvent Ãªtre nÃ©gative, dans ce cas la valeur represente la position depuis le bord droite et/ou bas
	* @return bool
	*/
	public static function toImg($nick, $type = 'png', $fontSize = 12, $forceDefault = false, $defaultColor = 'white', $defaultImgBackground = null, $arrRelativePosition = array() ){
		if(self::_isSupportedImageType($type) === false){
			$imErr = self::_getErrorRessource('Le type "'.$type.'" n\'est pas supportÃ©.');
	...
	*/
	...
	/**
	* Convertie une couleur au format HTML, TM, array RGB, ou nommÃ© en format HTML
	*
	* @param string $color
	* @return string
	*/
	...
	/**
	* Convertie une couleur au format HTML, TM, array RGB, ou nommÃ© en format RGB
	*
	* @param string $color
	* @return string
	*/
	...
	/**
	* Retourne le code css nÃ©cessaire Ã  la mise en forme du code html
	*
	* @return string
	*/
	...
	/**
	* EnlÃ¨ve tous les codes de trackmania
	*
	* @param string $str   -> Texte Ã  traiter
	* @param array $select -> Un ou plusieurs codes Ã  enlever. Par exemple, si on veut enlever seulement le code "$s"
	...
	*/
	...
	/**
	* Parse une chaine TM dans une structure de donnÃ©e interne utilisÃ© par les mÃ©thode toHtml, toImg, et toText
	*
	* @param string $nick la chaine TM pas de limite de longueur, et peut Ãªtre multiligne
	* @param string $defaultColor la couleur par dÃ©faut (la couleur peut Ãªtre au format HTML, nommÃ©, array RGB, ou TM)
	* @return array
	...
	*/
	...
	/**
	* DÃ©termine si le type passÃ© est supportÃ©
	*
	* @param string $type type supportÃ© (gif, jpg, jpeg, png, ico, bmp, wbmp, xpm)
	* @return bool
	*/
	...
	/**
	* Affiche l'image correspondant Ã  la ressource dans le format souhaitÃ©
	*
	* @return bool
	*/
	private static function _getImageFromRessource($im, $type){
		...
		return imagepng(self::_getErrorRessource('Le format "'.$type.'" n\'est pas supportÃ©.'));
	}
}
?>